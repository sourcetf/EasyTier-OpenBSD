...
impl AbstractDevice for Device {
    ...
    fn mtu(&self) -> Result<u16> {
        unsafe {
            let mut ti: tuninfo = mem::zeroed();
            if let Err(err) = tungifinfo(self.tun.as_raw_fd(), &mut ti) {
                return Err(io::Error::from(err).into());
            }
            ti.mtu.try_into().map_err(|_| Error::TryFromIntError)
        }
    }
    ...
    fn set_mtu(&mut self, value: u16) -> Result<()> {
        unsafe {
            ...
            ti.mtu = value as libc::c_uint;
            if let Err(err) = tunsifinfo(self.tun.as_raw_fd(), &ti) {
                return Err(io::Error::from(err).into());
            }
